...
const sections: Array<{ id: SectionId; label: string; icon: string; subtitle: string; sprite: number }> = [
  { id: "home", label: "Главная", icon: "⌂", subtitle: "Всё важное сейчас", character: "/glasha/characters/home.webp" },
  { id: "tasks", label: "Мои дела", icon: "✓", subtitle: "Личное и бытовое", character: "/glasha/characters/cooking.webp" },
  { id: "work", label: "Работа", icon: "▣", subtitle: "Проекты и задачи", character: "/glasha/characters/work.webp" },
  { id: "calendar", label: "Календарь", icon: "◫", subtitle: "События и напоминания", character: "/glasha/characters/quick.webp" },
  { id: "finance", label: "Финансы", icon: "₽", subtitle: "Личные и рабочие", character: "/glasha/characters/documents.webp" },
  { id: "health", label: "Здоровье", icon: "♡", subtitle: "Самочувствие и цикл", character: "/glasha/characters/health.webp" },
  { id: "learning", label: "Обучение", icon: "◉", subtitle: "Языки и развитие", character: "/glasha/characters/learning.webp" },
  { id: "travel", label: "Поездки", icon: "✈", subtitle: "Билеты и планы", character: "/glasha/characters/travel.webp" },
  { id: "documents", label: "Документы", icon: "▤", subtitle: "Всё под рукой", character: "/glasha/characters/documents.webp" },
  { id: "quick", label: "Быстрый доступ", icon: "⌘", subtitle: "Ссылки и приложения", character: "/glasha/characters/quick.webp" },
  { id: "advisor", label: "Советчик", icon: "?", subtitle: "Найти и разобраться", character: "/glasha/characters/ideas.webp" },
  { id: "chat", label: "Поговорить", icon: "✦", subtitle: "Выгрузить мысли", character: "/glasha/characters/cat.webp" },
];
  ...
  return <main className="appShell">
    ...
    <section className="workspace">
      ...
      {active === "home" ? <>
        ...
        </div><div className="heroGlasha"><GlashaCharacter src="/glasha/characters/home.webp" priority className="heroCharacter"/><span className="speechBubble">Я рядом ♡</span></div></section>
      ...
      </> : <SectionContent active={active} currentCharacter={current.character} tasks={tasks} events={events} expenses={expenses} notes={notes} totalExpenses={totalExpenses}
...
function SectionContent({ active, currentSprite, tasks, events, expenses, notes, totalExpenses, onToggleTask, onAddTask, onAddExpense, onCommand }:{
  active: SectionId; currentCharacter: string; tasks:Task[]; events:EventItem[]; expenses:Expense[]; notes:Note[]; totalExpenses:number;
  ...
  return <div className="sectionLayout">
    <section className="sectionLead"><div><Chip>{sections.find(s=>s.id===active)?.label}</Chip><h2>{headline(active)}</h2><p>{description(active)}</p></div><div className="sectionGlasha"><GlashaCharacter src={currentCharacter} className="sectionCharacter"/></div></section>
