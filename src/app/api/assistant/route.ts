...
async function parseOne(ctx: Context, text: string): Promise<Parsed> {
  ...
  const response = await ctx.openai.responses.create({
    ...
    instructions:
      ...
      "Личная задача -> create_task area=personal. Фраза 'по работе', а также очевидный рабочий контекст (клиент, бухгалтер, договор клиенту, рабочий проект) -> create_task area=work. " +
      "Если пользователь говорит 'завтра/в пятницу/в 15:00 позвонить человеку', это запланированная задача create_task, НЕ contact_action. " +
      "contact_action используй только для немедленной команды без будущей даты: 'позвони Кайрату', 'напиши Кайрату в Telegram'. " +
      ...
      "Вопрос 'что мне сделать для моей цели B1/цели X' -> advice и обязательно goal_title с названием цели. " +
